...
/// 等学习 Controller 完全不需要理解"自建"。
///
/// 字段以现有 [Word] 模型为兼容基准：
/// - wordId 使用 uuid，不随内容变化
/// - word 保存 trim + lowercase 标准化结果
@freezed
class CustomWordRecord with _$CustomWordRecord {
  const factory CustomWordRecord({
    ...
    /// 英文单词（trim + lowercase 标准化）。
    required String word,

    /// 音标（可选，缺省为空串）。
    required String phonetic,

    /// 释义列表（表单只输入中文，生成一条默认 MeaningEntry）。
