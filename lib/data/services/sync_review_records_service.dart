import 'word_book_id_map.dart';
import 'word_id_map.dart';
import '../../core/network/network_exception.dart';
import '../../domain/models/word_review_model.dart';
import '../../domain/services/review_sync_merger.dart';
import '../dto/review_record_dto.dart';
import '../repositories/review_repository.dart';
import '../repositories/review_sync_repository.dart';
...
/// ```
///
/// 原则 12：网络失败前不清空、不重建 Hive。
/// 原则 11：同步幂等 — 相同输入产生相同结果，不制造重复记录。
/// 写回 Hive 时保留服务器记录的 clientUpdatedAt，不刷新为 now()。
class SyncReviewRecordsService {
  SyncReviewRecordsService(
    ...
    try {
      ...
      if (mergeResult.recordsToUpload.isNotEmpty) {
        ...
        if (uploadDtos.isNotEmpty) {
          ...
          // 服务器返回 canonical records，写回 Hive
